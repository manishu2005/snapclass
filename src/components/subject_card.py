...
def subject_card(name, code, section, stats =None, footer_callback=None):
    ...
    if stats:
        ...
        for stat in stats:
            ...
            html += f'<div style="background:#F3F4F6; color:#111827; padding:5px 12px; border-radius:12px; font-size:0.9rem; border:1px solid rgba(0,0,0,0.08)">{icon} <b>{value}</b> {label}</div>'
        html+="</div>"
